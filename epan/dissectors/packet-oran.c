...
#define PAYLOAD_VERSION_LEGACY 0 // should be 1
#define PAYLOAD_VERSION_GSM 1    // to be fixed

#define GSM_LONG_PL_SIZE 552
#define GSM_PL_SIZE 548
#define GSM_PL_SIZE_DELTA (GSM_LONG_PL_SIZE - GSM_PL_SIZE)

#define GSM_LONG_SYM_DUR 0.071875 * 0.001
#define GSM_SYM_DUR 0.0713542 * 0.001
#define GSM_SYM_DUR_DELTA (GSM_LONG_SYM_DUR - GSM_SYM_DUR)
                                  ...
                                  unsigned num_sinr_per_prb)
{
    ...
    else if (sectionType == SEC_C_CH_INFO)
    { /* Section Type 6 */
        /* ef */
        proto_tree_add_item_ret_boolean(c_section_tree, hf_oran_ef, tvb, offset, 1, ENC_BIG_ENDIAN, &extension_flag);
        /* ueId */
    ...
    while (extension_flag)
    {
        ...
        case 6: /* SE 6: Non-contiguous PRB allocation in time and frequency domain */
        {
            ...
            case 0:
                ...
                break;
            case 1:
                ext11_settings.ext6_rbg_size = 1;
                break;
            case 2:
                ext11_settings.ext6_rbg_size = 2;
                break;
            case 3:
                ext11_settings.ext6_rbg_size = 3;
                break;
            case 4:
                ext11_settings.ext6_rbg_size = 4;
                break;
            case 5:
                ext11_settings.ext6_rbg_size = 6;
                break;
            case 6:
                ext11_settings.ext6_rbg_size = 8;
                break;
            case 7:
                ext11_settings.ext6_rbg_size = 16;
                break;
            ...
            if (!disableBFWs)
            {
                ...
                for (unsigned b = 0; b < num_bundles; b++)
                {
                    ...
                    offset = dissect_bfw_bundle(tvb, extension_tree, pinfo, offset,
                                                ...
                                                (ext11_settings.ext21_set) ? numPrbc : pref_num_weights_per_bundle,
                                                bfwcomphdr_iq_width,
                                                b, /* bundle number */
...
static unsigned dissect_payload_version(proto_tree *tree, tvbuff_t *tvb, packet_info *pinfo, unsigned offset)
{
    unsigned version;
    proto_item *ti = proto_tree_add_item_ret_uint(tree, hf_oran_payload_version, tvb, offset, 1, ENC_NA, &version);
    if ((version != PAYLOAD_VERSION_LEGACY) && (version != PAYLOAD_VERSION_GSM))
    {
                                         ...
                                         ack_nack_request_t *request)
{
    ...
    int seconds_between_packets = (int)(pinfo->abs_ts.secs - request->request_frame_time.secs);
                                          ...
                                          ack_nack_request_t *response)
{
    ...
    /* Work out gap between frames (in ms) */
    int seconds_between_packets = (int)(response->response_frame_time.secs - pinfo->abs_ts.secs);
    int nseconds_between_packets =
...
static int dissect_oran_c(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, void *data _U_)
{
    ...
    if ((sectionType == SEC_C_ACK_NACK_FEEDBACK) || /* Section Type 8 */
        (sectionType == SEC_C_SINR_REPORTING))
    { /* Section Type 9 */
        /* symbolId */
...
static int dissect_oran_u_gsm(tvbuff_t *tvb, packet_info *pinfo,
                              proto_item *protocol_item, proto_tree *oran_tree,
                              proto_item *timingHeader, proto_tree *timing_header_tree,
                              uint32_t direction, int ref_a_offset);

static int dissect_oran_u(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, void *data _U_)
{
    /* Hidden filter for plane */
    ...
    /* payloadVersion */
    unsigned payload_version = dissect_payload_version(timing_header_tree, tvb, pinfo, offset);
    /* filterIndex */
    proto_tree_add_item(timing_header_tree, hf_oran_filter_index, tvb, offset, 1, ENC_NA);
    offset += 1;
    if (payload_version == PAYLOAD_VERSION_GSM)
    {
        col_set_str(pinfo->cinfo, COL_PROTOCOL, "O-RAN-FH-U-GSM");
        col_set_str(pinfo->cinfo, COL_INFO, "U-Plane-GSM");
        proto_item_append_text(protocol_item, "-GSM");

        return dissect_oran_u_gsm(tvb, pinfo, protocol_item, oran_tree, timingHeader, timing_header_tree, direction, offset);
    }
...
float computeTime(int frame, int subframe, int symbol)
{
    float s = symbol * GSM_SYM_DUR;

    if (symbol > 7)
        s += 2 * GSM_SYM_DUR_DELTA;
    else if (symbol)
        s += GSM_SYM_DUR_DELTA;

    s += frame * 0.01 + subframe * 0.001;
    return s;
}
                              ...
                              uint32_t direction, int ref_a_offset)
{
    ...
    char id[64];
    snprintf(id, sizeof(id), "+%f %s (%d-%d-%d-%d)",
             computeTime(frameId, subframeId, symbolId),
             val_to_str_const(direction, data_direction_vals, "?"),
