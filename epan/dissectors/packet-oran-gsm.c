...
#define GSM_LONG_SYM_DUR 0.071875
#define GSM_SYM_DUR 0.0713542
...
static const value_string data_direction_vals[] = {
    { DIR_UPLINK,   "UL" },   /* gNB Rx */
    { DIR_DOWNLINK, "DL" }, /* gNB Tx */
    { 0, NULL}
};
...
static float computeTime(int frame, int subframe, int symbol){
    float s = symbol*GSM_SYM_DUR;

    if (symbol > 7) s += 2*GSM_SYM_DUR_DELTA;
    else if (symbol) s+= GSM_SYM_DUR_DELTA;
...
dissect_oran_u_gsm(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, void *data _U_)
{
    ...
    int samples = (tvb_captured_length(tvb) - offset)>>1; // 16bits samples
    int samplesExpected = GSM_PL_SIZE;
    if (symbolId> 7) samplesExpected += 2*GSM_PL_SIZE_DELTA;
    else if(symbolId) samplesExpected += GSM_PL_SIZE_DELTA;

    samplesExpected *= GSM_SAMPLE_RATE_MULT;

    if (samplesExpected == samples){
        proto_item_append_text(protocol_item, "%d IQ samples", samples);
        return tvb_captured_length(tvb);
    } 
    if (samples > samplesExpected){
        proto_item_append_text(protocol_item, "%d IQ samples, %d bytes extra", samplesExpected, (samples-samplesExpected)<<1);
        return tvb_captured_length(tvb);        
    }
