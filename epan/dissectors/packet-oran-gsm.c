...
static int
dissect_oran_u_gsm(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, void *data _U_)
{
    /* Hidden filter for plane */
    ...
    /* Create display subtree for the protocol */
    proto_item *protocol_item = proto_tree_add_item(tree, proto_oran, tvb, 0, -1, ENC_NA);
    proto_item_append_text(protocol_item, "-U-GSM");
    proto_tree *oran_tree = proto_item_add_subtree(protocol_item, ett_oran);
    ...
    proto_tree *timing_header_tree = proto_tree_add_subtree(oran_tree, tvb, offset, 4, ett_oran_u_timing, &timingHeader, "Timing (");
...
dissect_oran(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, void *data)
{
    ...
    case ECPRI_MT_IQ_DATA:
        return dissect_oran_u_gsm(tvb, pinfo, tree, data);
